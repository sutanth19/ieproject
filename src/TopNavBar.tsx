...
function Navigation(): React.JSX.Element {
  ...
  const naviPage: NavPage[] = [
    ...
    },
    {
      ...
      child: [
        { 
          title: 'Workday', 
          thumbnail: '', 
          url: '/training/workday',
          externalUrl: 'https://wd5.myworkday.com/jabil/learning',
          isExternal: true 
        },
        { 
          title: 'IE Webinars', 
          thumbnail: '', 
          url: '/training/webbinar', 
          externalUrl: 'https://jabil.sharepoint.com/sites/IEPortal/SitePages/IE-Webinars.aspx',
          isExternal: true 
        },
        { 
          title: 'e-Jabilization', 
          thumbnail: '', 
          url: '/training/ejabilization',
          externalUrl: 'http://jpertdf01/eJabilization/UserLogin.aspx',
          isExternal: true 
        },
      ],
    },
    ...
    },
    {
      ...
      child: [
        { 
          title: 'Site Master Layout', 
          thumbnail: '', 
          url: '/penweb/sitemasterlayout', 
          externalUrl: 'https://jabil-my.sharepoint.com/:f:/p/mohamad_syafezy/EuNIeEHcmPBKoig7Gd3LlekB6P5XkE1H-Kc7nqk9QcFcrQ?e=wT6Bm8',
          isExternal: true 
        },
        { 
          title: 'Production Cookbook Line Setup', 
          thumbnail: '', 
          url: '/penweb/productioncookbooklinesetup', 
        ...
        { 
          title: 'Site Area Statement', 
          thumbnail: '', 
          url: '/penweb/siteareastatement', 
  ...
  const isActiveParent = (parent: string, child?: NavChild[]): boolean => {
    ...
    if (child && child.length > 0) {
      return child.some((childItem: NavChild) => {
  ...
  const handleNavigation = (url: string, isExternal?: boolean, externalUrl?: string): void => {
...
function IconThemeToggle({ mode, onClick }: IconThemeToggleProps): React.JSX.Element {
  const icon = mode ? <LightModeIcon /> : <DarkModeIcon />;
  const tooltipText = mode ? "Switch to Light Mode" : "Switch to Dark Mode";
...
function LogoWithNavigation(): React.JSX.Element {
  const navigate = useNavigate();
...
function AppsToggle(): React.JSX.Element {
  const { darkMode } = useTheme() as ThemeContextType;
  const navigate = useNavigate();
  ...
  return (
    <React.Fragment>
      <Tooltip title="Apps" placement="left" arrow>
          ...
          >
            {appList && appList.length > 0 ? (
              <Grid container spacing={1}>
                {appList.map((item: AppItem, index: number) => (
...
function TopNavBar(): React.JSX.Element {
  ...
  const isActiveLink = (path: string, sectionId?: string): boolean => {
    const currentPath = location.pathname.toLowerCase();
  ...
  const handleNavigation = (path: string, sectionId?: string): void => {
    setDrawerOpen(false);
