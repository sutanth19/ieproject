...
const TCCS = () => {
  ...
  const fetchCarouselItems = useCallback(async (isInitialLoad = false) => {
    ...
    try {
      const response = await fetch(`${API_URL}?type=TCCS&timestamp=${new Date().getTime()}`);
      if (!response.ok) {
      ...
      if (isInitialLoad && carouselItems.length === 0) {
        setCarouselItems(getDefaultTCCSSlides());
      }
    } finally {
      if (isInitialLoad) {
        setLoading(false);
  ...
  const getDefaultTCCSSlides = () => [
    {
      image: workdaySlide1,
      title: 'TCCS Training',
      description: 'TCCS is designed to systematically manage the learning requirement baseline, certification management, competency, and skill management.'
    },
    {
      image: workdaySlide2,
    ...
    <TrainingLayout
      ...
      CarouselComponent={
        loading ? (
          ...
          <TrainingCarousel 
            slides={carouselItems.length > 0 ? carouselItems : getDefaultTCCSSlides()} 
            carouselAriaLabel="TCCS Training Carousel" 
            key={lastUpdate} // Force re-render when data updates
          />
        )
      }
    >
      {/* Optionally, add additional TCCS-specific content here */}
    </TrainingLayout>
  );
};
